...
PyEnvironment::mutateGlobalVar(PyConstants::VarTypes vartype, const std::string &varName, boost::any value) {
    switch ( vartype ) {
        case PyConstants::VarTypes::NUMBER: {
            std::unique_ptr<PyFloat> pyFloat = std::unique_ptr<PyFloat>(new PyFloat(boost::any_cast<double>(value)));
            globalVars.at(varName) = std::move(pyFloat);
            break;
        }
...
void PyEnvironment::modifyGlobalVar(PyConstants::VarTypes vartype, const std::string &varName, boost::any value) {
    switch ( vartype ) {
        case PyConstants::VarTypes::NUMBER: {
            globalVars.at(varName)->setData<double>(boost::any_cast<double>(value));
            break;
        }
        case PyConstants::VarTypes::BOOL: {
    ...
    switch ( vartype ) {
        case PyConstants::VarTypes::NUMBER: {
            globalVars.insert(
                    std::make_pair(
                            varName,
                            std::unique_ptr<PyFloat>(new PyFloat(boost::any_cast<double>(value)))
                    ));
            break;
        }
        case PyConstants::VarTypes::BOOL: {
